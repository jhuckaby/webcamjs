...
var Webcam = {
	...
	reset: function() {
		try {
			...
			if (!this.hasUserMedia() && this.hasFlash()) {
	...
	dispatch: function() {
		...
		if (this.hooks[name] && this.hooks[name].length) {
			for (var idx = 0, len = this.hooks[name].length; idx < len; idx++) {
				...
				} else if (hookType == 'object' && hook.length == 2) {
	...
	/*
	 * this.hasUserMedia() to check if html5 video is supported
	 * this.hasUserMedia(true) can not be tricked by force_flash.
	 */
	hasUserMedia: function(includeCachedValue) {
		return this.userMedia || (includeCachedValue ? _userMedia : false);
	},
	...
	hasFlash: function() {
		...
		if (nav.plugins !== undefined && typeof nav.plugins[SHOCKWAVE_FLASH] === "object") {
...
}(typeof window === 'undefined' ? nonBrowser : window));
